...
io.on('connection', (socket) => {
  ...
  socket.on('create_game', ({ quiz, hostName }) => {
    ...
    console.log(`🎮 Partie créée: ${code} par ${hostName}`);
  });
  ...
  socket.on('start_game', ({ gameCode }) => {
    ...
    if (game.players.length === 0) return socket.emit('error', '👥 Attendez des joueurs !');
    game.state = 'playing';
...
<head>
    ...
    <title>Quiz App - Créez et jouez en direct</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: -apple-system, 'BlinkMacSystemFont', 'Segoe UI', 'Roboto', 'Helvetica Neue', sans-serif; }
        ...
        /* Glassmorphism - Style AIR */
        .glass { background: rgba(255, 255, 255, 0.2); backdrop-filter: blur(20px); border-radius: 32px; border: 1px solid rgba(255,255,255,0.3); padding: 28px; margin-bottom: 24px; box-shadow: 0 8px 32px rgba(0,0,0,0.1); }
        .glass-card { background: rgba(255, 255, 255, 0.15); backdrop-filter: blur(12px); border-radius: 24px; padding: 20px; border: 1px solid rgba(255,255,255,0.2); transition: all 0.3s ease; }
        ...
        .mode-btn { background: rgba(255,255,255,0.15); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.3); padding: 14px 32px; border-radius: 60px; font-weight: 600; font-size: 1rem; cursor: pointer; color: white; transition: all 0.3s; }
        .mode-btn.active { background: white; color: #667eea; border-color: white; box-shadow: 0 8px 25px rgba(0,0,0,0.2); }
        
        button { background: linear-gradient(135deg, #667eea, #764ba2); border: none; padding: 12px 24px; border-radius: 50px; font-weight: 600; color: white; cursor: pointer; font-size: 0.95rem; transition: all 0.2s; }
        button:hover { transform: scale(1.02); box-shadow: 0 8px 20px rgba(0,0,0,0.2); }
        button.secondary { background: rgba(255,255,255,0.2); backdrop-filter: blur(10px); }
        button.success { background: linear-gradient(135deg, #00b4db, #0083b0); }
        
        input, textarea, select { background: rgba(255,255,255,0.9); border: none; border-radius: 20px; padding: 14px 18px; width: 100%; margin-bottom: 15px; font-size: 1rem; outline: none; }
        ...
        .option-btn { background: rgba(255,255,255,0.2); backdrop-filter: blur(10px); padding: 16px; border-radius: 60px; font-weight: 600; text-align: center; cursor: pointer; transition: all 0.2s; border: 1px solid rgba(255,255,255,0.3); color: white; }
        .option-btn:hover { background: rgba(255,255,255,0.4); transform: scale(1.02); }
        
        .timer-bar { height: 8px; background: rgba(255,255,255,0.3); border-radius: 10px; margin: 15px 0; overflow: hidden; }
        .timer-fill { height: 100%; background: white; width: 100%; border-radius: 10px; transition: width 0.1s linear; }
        
        .code-display { font-size: 2.5rem; letter-spacing: 12px; background: rgba(0,0,0,0.3); backdrop-filter: blur(10px); padding: 20px; border-radius: 20px; font-family: monospace; font-weight: bold; text-align: center; color: white; }
        
        .player-list { background: rgba(255,255,255,0.1); border-radius: 20px; padding: 15px; margin-top: 15px; }
        .player-item { padding: 10px; border-bottom: 1px solid rgba(255,255,255,0.2); display: flex; justify-content: space-between; color: white; }
        
        .chat-window { background: rgba(255,255,255,0.1); backdrop-filter: blur(20px); border-radius: 24px; height: 450px; display: flex; flex-direction: column; overflow: hidden; }
        .chat-messages { flex: 1; padding: 20px; overflow-y: auto; display: flex; flex-direction: column; gap: 12px; }
        .bot-msg, .user-msg { max-width: 80%; padding: 12px 18px; border-radius: 20px; font-size: 0.95rem; }
        .bot-msg { background: rgba(102,126,234,0.9); color: white; align-self: flex-start; border-bottom-left-radius: 5px; }
        .user-msg { background: rgba(255,255,255,0.3); color: white; align-self: flex-end; border-bottom-right-radius: 5px; }
...
<div class="container">
    <div class="mode-bar">
        <button class="mode-btn active" id="btnHostMode">🎮 Organiser</button>
    ...
    <div id="hostPanel">
        <div class="glass">
            ...
            <div style="display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 20px;">
                ...
                <button id="chatbotCreateBtn" class="secondary">🤖 Chatbot intelligent</button>
                <button id="manualCreateBtn" class="secondary">📝 Manuel (1-40 questions)</button>
            </div>
            <h3>📚 Mes quiz sauvegardés</h3>
...
<script>
    ...
    function renderQuizList() {
        ...
        container.innerHTML = quizzes.map((quiz, idx) => \`
            <div class="glass-card" style="cursor:pointer;">
                ...
                <p style="margin:10px 0; opacity:0.8;">📋 \${quiz.questions.length} questions</p>
                <button class="host-quiz-btn" data-idx="\${idx}">🚀 Héberger ce quiz</button>
            </div>
        \`).join('');
        document.querySelectorAll('.host-quiz-btn').forEach(btn => {
            btn.addEventListener('click', (e) => { startHostingQuiz(parseInt(btn.dataset.idx)); });
        });
    }
    
    // Création rapide IA
    function quickCreate() {
        let theme = prompt("🎯 Thème du quiz (ex: 'Marketing', 'Économie', 'Droit')", "Culture STMG");
        if (!theme) return;
        let nb = parseInt(prompt("📊 Nombre de questions (1 à 40)", "5"));
        if (isNaN(nb)) nb = 5;
        ...
        for (let i = 0; i < nb; i++) {
            questions.push({
                text: \`Question \${i+1} sur \${theme} : Quel est le concept clé ?\`,
        ...
        saveQuizzes();
        renderQuizList();
        alert(\`✅ Quiz "\${theme}" créé avec \${nb} questions !\`);
    }
    
    // Chatbot intelligent
    function openChatbot() {
        const modal = document.createElement('div');
        modal.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.8);backdrop-filter:blur(12px);z-index:1000;display:flex;align-items:center;justify-content:center;';
        modal.innerHTML = \`
            <div class="glass" style="width:90%;max-width:550px;">
                <h2>🤖 Chatbot QuizMaster</h2>
                <div class="chat-window">
                    <div class="chat-messages" id="chatbotMsgs">
                        <div class="bot-msg">✨ Salut ! Je crée des quiz pour toi en quelques secondes.</div>
                        <div class="bot-msg">Donne-moi : matière + nombre de questions (ex: "Histoire STMG 10")</div>
                    </div>
        ...
        function generateQuiz(promptText) {
            ...
            let nb = nbMatch ? parseInt(nbMatch[0]) : 5;
            nb = Math.min(40, Math.max(1, nb));
            let theme = promptText.replace(/\\d+/g, '').trim() || "Culture générale";
            let questions = [];
            for (let i = 0; i < nb; i++) {
                questions.push({
                    text: \`Question \${i+1} : Concept important en \${theme} ?\`,
        ...
        function processAnswer(answer) {
            ...
            addBotMsg(\`🎉 Quiz "\${result}" créé ! Retrouve-le dans "Mes quiz".\`);
            setTimeout(() => modal.remove(), 2000);
        }
        
        document.getElementById('sendChatbotMsg').onclick = () => { let val = input.value.trim(); if(val) processAnswer(val); };
    ...
    // Création manuelle (1-40 questions)
    function openManualCreator() {
        let nb = parseInt(prompt("📊 Combien de questions ? (1 à 40)", "5"));
        if (isNaN(nb)) nb = 5;
        nb = Math.min(40, Math.max(1, nb));
        ...
        for (let i = 0; i < nb; i++) {
            ...
            let opts = [];
            for (let j = 0; j < 4; j++) {
                let opt = prompt(\`Option \${String.fromCharCode(65+j)} pour la question \${i+1} :\`, \`Option \${String.fromCharCode(65+j)}\`);
                if (!opt) opt = \`Option \${String.fromCharCode(65+j)}\`;
                opts.push(opt);
            }
            let correct = parseInt(prompt("Bonne réponse (1,2,3 ou 4):", "1")) - 1;
        ...
        if (questions.length > 0) {
            ...
            saveQuizzes();
            renderQuizList();
            alert(\`✅ Quiz "\${title}" créé avec \${questions.length} questions !\`);
        }
    }
    ...
    function startHostingQuiz(quizIndex) {
        ...
        socket.once('game_created', (data) => {
            ...
            document.getElementById('hostRoomInfo').innerHTML = \`
                <div style="text-align:center;">
                    ...
                    <p style="margin:15px 0;">Partagez ce code avec vos élèves !</p>
                    <button id="startGameBtn" class="success">🚀 Démarrer le quiz</button>
                </div>
            \`;
        ...
        socket.on('players_update', (players) => {
            document.getElementById('playersList').innerHTML = \`<h4>👥 Joueurs (\${players.length})</h4>\` + players.map(p => \`<div class="player-item"><span>\${escapeHtml(p.name)}</span><span>⭐ \${p.score} pts</span></div>\`).join('');
        });
        
        socket.on('game_started', (data) => { document.getElementById('hostRoomInfo').innerHTML = \`<h3>🎮 Partie en cours - \${data.total} questions</h3>\`; });
        
        socket.on('new_question', (data) => {
            document.getElementById('gameControls').innerHTML = \`
                <div style="background:rgba(255,255,255,0.1);border-radius:28px;padding:25px;text-align:center;">
            ...
            let timerFill = document.querySelector('.timer-fill');
            let interval = setInterval(() => { timeLeft--; if(timerFill && timeLeft>=0) timerFill.style.width = \`\${(timeLeft/data.timeLimit)*100}%\`; if(timeLeft<=0) clearInterval(interval); }, 1000);
        });
        ...
        socket.on('game_ended', (data) => {
            document.getElementById('gameControls').innerHTML = \`
                ...
                <button onclick="location.reload()" style="margin-top:20px;">➕ Nouveau quiz</button>
            \`;
        });
        
        socket.on('host_disconnected', () => { alert('❌ Partie fermée'); location.reload(); });
        socket.on('error', (msg) => alert(msg));
    }
    
    function joinGame() {
        let gameCode = document.getElementById('joinCode').value.trim().toUpperCase();
        let playerName = document.getElementById('playerName').value.trim();
        if (!gameCode || !playerName) { alert('Code et pseudo requis'); return; }
        ...
        socket.once('joined', (data) => {
            ...
            document.getElementById('gameStatus').innerHTML = \`<div style="background:#00c853;padding:12px;border-radius:20px;">✅ Connecté à "\${data.quizTitle}" - En attente...</div>\`;
        });
        
        socket.on('game_started', (data) => { document.getElementById('gameStatus').innerHTML = \`<div style="background:#667eea;padding:12px;border-radius:20px;">🎯 Quiz démarré ! \${data.total} questions</div>\`; });
        
        socket.on('new_question', (data) => {
            document.getElementById('questionArea').innerHTML = \`
                <div style="background:rgba(255,255,255,0.1);border-radius:28px;padding:25px;">
                    <h3>Question \${data.qIndex}/\${data.total}</h3>
                    <h2>\${escapeHtml(data.question)}</h2>
                    <div class="timer-bar"><div class="timer-fill" style="width:100%"></div></div>
            ...
            let timeLeft = data.timeLimit;
            let timerFill = document.querySelector('.timer-fill');
            let interval = setInterval(() => { timeLeft--; if(timerFill && timeLeft>=0) timerFill.style.width = \`\${(timeLeft/data.timeLimit)*100}%\`; if(timeLeft<=0) { clearInterval(interval); grid.querySelectorAll('button').forEach(b => b.disabled = true); } }, 1000);
        });
        
        socket.on('answer_result', (data) => {
            let fb = document.createElement('div');
            fb.className = data.correct ? 'correct-feedback' : 'wrong-feedback';
            fb.innerHTML = data.correct ? \`✅ Bonne réponse ! +\${data.points} points\` : \`❌ Mauvaise. La réponse était : \${data.correctAnswer}\`;
        ...
        socket.on('game_ended', (data) => {
            document.getElementById('questionArea').innerHTML = \`
                <h2>🎉 Quiz terminé !</h2>
                \${data.scores.map(s => \`<div class="player-item"><span>\${escapeHtml(s.name)}</span><span>\${s.score} pts</span></div>\`).join('')}
                <button onclick="location.reload()" style="margin-top:20px;">🎮 Rejoindre un autre quiz</button>
            \`;
        });
        
        socket.on('host_disconnected', () => { alert('❌ L\\'organisateur a quitté'); location.reload(); });
        socket.on('error', (msg) => alert(msg));
    }
